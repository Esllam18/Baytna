...
def upgrade() -> None:
    ...
    # ---------------------------------------------------------------
    with op.batch_alter_table("zone_traffic_policies", recreate="always") as batch:
        batch.add_column(
            sa.Column(
    ...
    with op.batch_alter_table("expansion_rollout_events", recreate="always") as batch:
        batch.add_column(
            sa.Column(
                "trigger_source",
                sa.String(length=20),
    ...
    # ---------------------------------------------------------------
    with op.batch_alter_table("daily_financial_closes", recreate="always") as batch:
    ...
    # ---------------------------------------------------------------
    with op.batch_alter_table("launch_evidence_packs", recreate="always") as batch:
        batch.add_column(
            sa.Column(
                "retention_class",
                sa.String(length=20),
